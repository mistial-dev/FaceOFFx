...
public static class PivLandmarkProcessor
{
    ...
    public static async Task<Result<PivLandmarkResult>> ProcessAsync(
        ...
        ILogger? logger = null)
    {
        logger?.LogDebug("Starting PIV landmark processing for face: {Face}", detectedFace.BoundingBox);
        try
        {
            ...
            var landmarksResult = await landmarkExtractor.ExtractLandmarksAsync(sourceImage, detectedFace.BoundingBox).ConfigureAwait(false);
            if (landmarksResult.IsFailure)
            {
                logger?.LogWarning("Landmark extraction failed: {Error}", landmarksResult.Error);
                return Result.Failure<PivLandmarkResult>($"Landmark extraction failed: {landmarksResult.Error}");
            }

            var originalLandmarks = landmarksResult.Value;
            logger?.LogDebug("Successfully extracted {Count} landmarks", originalLandmarks.Points.Count);
            ...
            var rotatedLandmarks = RotateLandmarks(originalLandmarks, rotationDegrees, sourceImage.Width, sourceImage.Height, logger);
            ...
            var faceCropResult = CalculateFaceCrop(rotatedLandmarks, rotatedImage.Width, rotatedImage.Height, logger);
            if (faceCropResult.IsFailure)
            {
                logger?.LogWarning("Face crop calculation failed: {Error}", faceCropResult.Error);
                rotatedImage?.Dispose();
                return Result.Failure<PivLandmarkResult>($"Face crop calculation failed: {faceCropResult.Error}");
            }

            var faceCrop = faceCropResult.Value;
            logger?.LogDebug("Face crop region: x={X}, y={Y}, width={Width}, height={Height}", 
                faceCrop.X, faceCrop.Y, faceCrop.Width, faceCrop.Height);
            ...
            var complianceValidation = PivComplianceValidation.Validate(pivLines, pivImage.Width, pivImage.Height);
            
            logger?.LogDebug("PIV Lines calculated - AA: {AA:F1}, BB: {BB:F1}, CC: {CC:F1}", 
                pivLines.LineAA_X, pivLines.LineBB_Y, pivLines.LineCC_Width);
            ...
            // Step 9: Calculate ROI regions using Appendix C.6 approach
            logger?.LogDebug("Step 9: Calculating Appendix C.6 ROI regions");
            ...
            if (roiResult.IsFailure)
            {
                ...
                return Result.Failure<PivLandmarkResult>($"ROI calculation failed: {roiResult.Error}");
            }

            var roiSet = roiResult.Value;
            logger?.LogDebug("Successfully calculated {Count} ROI regions", roiSet.AllRegions.Count);
            ...
            logger?.LogInformation("PIV landmark processing completed. Summary: {Summary}", result.ProcessingSummary);
            return Result.Success(result);
        }
        catch (Exception ex)
        {
            logger?.LogError(ex, "PIV landmark processing failed with exception");
            return Result.Failure<PivLandmarkResult>($"PIV landmark processing failed: {ex.Message}");
        }
    }
    ...
    /// </summary>
    private static float CalculateRotationFromLandmarks(FaceLandmarks68 landmarks, ILogger? logger = null)
    {
        var leftEye = landmarks.LeftEyeCenter;
        var rightEye = landmarks.RightEyeCenter;
        logger?.LogDebug("Eye centers - Left: ({LeftX}, {LeftY}), Right: ({RightX}, {RightY})", 
        ...
        {
            logger?.LogDebug("Rotation limited from {Original}° to {Limited}° for PIV compliance", 
                rotationDegrees, limitedRotation);
        }
    ...
    /// </summary>
    private static FaceLandmarks68 RotateLandmarks(FaceLandmarks68 landmarks, float rotationDegrees, int imageWidth, int imageHeight, ILogger? logger = null)
    {
        ...
        logger?.LogDebug("Rotating {Count} landmarks by {Degrees}°", landmarks.Points.Count, rotationDegrees);
        ...
        logger?.LogDebug("Image dimensions after rotation: {OldWidth}x{OldHeight} -> {NewWidth}x{NewHeight}", 
    ...
    /// </summary>
    private static Result<Rectangle> CalculateFaceCrop(FaceLandmarks68 landmarks, int imageWidth, int imageHeight, ILogger? logger = null)
    {
        try
        {
            logger?.LogDebug("Starting PIV-compliant face crop calculation with image dimensions: {Width}x{Height}", imageWidth, imageHeight);
            ...
            logger?.LogDebug("PIV Lines - AA (center): {AA:F1}, BB (eyes): {BB:F1}, CC (width): {CC:F1}", 
                pivLines.LineAA_X, pivLines.LineBB_Y, pivLines.LineCC_Width);
            ...
            logger?.LogDebug("Target head width in final image: {TargetWidth:F1}px (range: {MinWidth:F1}px - {MaxWidth:F1}px)", 
                targetHeadWidthInFinal, minHeadWidth, maxHeadWidth);
            ...
            logger?.LogDebug("Scale factor from current head ({Current:F1}px) to target ({Target:F1}px): {Scale:F3}", 
                pivLines.LineCC_Width, targetHeadWidthInFinal, scaleToTarget);
            ...
            logger?.LogDebug("Calculated crop dimensions: {Width:F1}x{Height:F1} (aspect: {Aspect:F3})", 
            ...
            var targetEyeFromTopRatio = 1.0f - PivComplianceValidation.Thresholds.OptimalEyeFromBottom; // 0.4 (40% from top)
            ...
            logger?.LogDebug("Crop positioning - Center: ({X:F1}, {Y:F1}), Target eye Y in crop: {EyeY:F1}", 
                cropCenterX, cropCenterY, targetEyeYInCrop);
            ...
            logger?.LogDebug("Final crop bounds - X: {X}-{MaxX} ({Width}px), Y: {Y}-{MaxY} ({Height}px)", 
                cropX, cropMaxX, finalCropWidth, cropY, cropMaxY, finalCropHeight);
            ...
            {
                logger?.LogWarning("Calculated crop region too small: {Width}x{Height} (minimum: {MinWidth}x{MinHeight})", 
                    finalCropWidth, finalCropHeight, minCropWidth, minCropHeight);
                return Result.Failure<Rectangle>($"Calculated crop region too small for PIV compliance: {finalCropWidth}x{finalCropHeight}");
            }
            
            // Step 9: Log boundary constraints if any
            if (cropX == 0 || cropY == 0 || cropMaxX == imageWidth || cropMaxY == imageHeight)
            {
                logger?.LogWarning("Crop was constrained by image boundaries - this may affect PIV compliance");
                logger?.LogDebug("Boundary constraints - Left: {Left}, Top: {Top}, Right: {Right}, Bottom: {Bottom}",
                    cropX == 0, cropY == 0, cropMaxX == imageWidth, cropMaxY == imageHeight);
            }
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure<Rectangle>($"PIV-compliant crop calculation failed: {ex.Message}");
        }
    }
        ...
        ILogger? logger = null)
    {
        ...
        logger?.LogDebug("Scale factors - X: {ScaleX}, Y: {ScaleY}, Final: {Scale}", scaleX, scaleY, scale);
    ...
    /// <summary>
    /// Calculates ROI regions using Appendix C.6 rectangular approach.
    /// This replaces the complex landmark-based ROI with a standards-compliant rectangular region.
    /// </summary>
    private static Result<FacialRoiSet> CalculateRoiFromPivLandmarks(FaceLandmarks68 landmarks, ILogger? logger = null)
    {
        try
        {
            logger?.LogDebug("Calculating Appendix C.6 compliant ROI regions");
            ...
            if (result.IsSuccess)
            {
                logger?.LogDebug("Appendix C.6 ROI calculation successful using standardized method");
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure<FacialRoiSet>($"Appendix C.6 ROI calculation failed: {ex.Message}");
        }
    }
