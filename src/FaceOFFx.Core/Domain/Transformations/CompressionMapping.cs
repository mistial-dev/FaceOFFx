...
public static class CompressionMapping
{
    ...
    /// </remarks>
    public static readonly (float Rate, int ApproxSizeBytes, string Description)[] RateToSizeMap =
        new[]
        {
    ...
    public static int GetExpectedSizeForRate(float rate)
    {
        ...
        foreach (
            var mapping in RateToSizeMap.Where(mapping => Math.Abs(mapping.Rate - rate) < 0.001f)
        )
        {
