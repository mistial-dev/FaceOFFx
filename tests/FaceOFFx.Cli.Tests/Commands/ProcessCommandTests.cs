...
public class ProcessCommandTests : IntegrationTestBase
{
    ...
    public override void OneTimeSetUp()
    {
        ...
        _cliProjectPath = Path.Combine(searchDir.FullName, "src", "FaceOFFx.Cli", "FaceOFFx.Cli.csproj");
    }
    ...
    public async Task ProcessCommand_WithValidImage_ProducesOutput()
    {
        ...
        var exitCode = await RunCliCommand($"process \"{_testImagePath}\" --output \"{outputPath}\"");
        ...
        fileInfo.Length.Should().BeInRange(10_000, 50_000, "JP2 file should be between 10KB and 50KB");
    }
    ...
    public async Task ProcessCommand_WithNoRoi_CreatesCorrectFormat()
    {
        ...
        var exitCode = await RunCliCommand($"process \"{_testImagePath}\" --output \"{outputPath}\" --no-roi");
    ...
    public async Task ProcessCommand_WithRateParameter_AppliesCompression()
    {
        ...
        await RunCliCommand($"process \"{_testImagePath}\" --output \"{highQualityPath}\" --rate 1.0");
        await RunCliCommand($"process \"{_testImagePath}\" --output \"{lowQualityPath}\" --rate 0.6");
        var highQualitySize = new FileInfo(highQualityPath).Length;
        var lowQualitySize = new FileInfo(lowQualityPath).Length;
        
        highQualitySize.Should().BeGreaterThan(lowQualitySize, 
    ...
    public async Task ProcessCommand_WithNoFaceInImage_ReturnsError()
    {
        ...
        var exitCode = await RunCliCommand($"process \"{noFaceImagePath}\" --output \"{outputPath}\"");
    ...
    public async Task ProcessCommand_WithoutOutputPath_UsesDefaultNaming()
    {
        ...
        var expectedOutputPath = Path.Combine(Path.GetDirectoryName(tempImagePath)!, expectedOutputName);
